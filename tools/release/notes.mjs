...
// taking has to guess which titles changed the program they run. gdharness found the cost of
// that guess: two projects downstream of it read the flat list on the same day, upgraded for a
// fixture that changed nothing they install, and both missed the release that fixed their
// problem. So the split is computed from the files each pull request touched, not from its
// title or a label somebody remembers to add.
//
...
// file picks the compiler, and release-build.yml assembles the archive and says what goes in.
const SHIPPED = [
  "crates/",
  "Cargo.toml",
  "Cargo.lock",
