...
def index(found: list[dict], shipped: str | None, to: Path) -> None:
    ...
    the repository, who will not think to run a script. Generated rather than kept by hand for
    the same reason every other stamp here is: a list somebody maintains is a list that is
    wrong by the second experiment.
    """
    # What a run changed is the only thing that makes its name unnecessary, and the name is
    # what nobody can look up. Measured against the commonest configuration rather than a
    # chosen one, so the baseline is whatever most of the work was done at.
    usual = Counter(
        tuple((key, str(setting(run, key))) for key in SETTINGS if key != "backbone")
        for run in found
    ).most_common(1)[0][0]
    baseline = dict(usual)

    rows = []
    for run in sorted(found, key=lambda run: run["id"]):
        result = run.get("validation") or {}
        changed = [
            shown(key, setting(run, key))
            for key in SETTINGS
            if key != "backbone" and str(setting(run, key)) != baseline.get(key)
        ]
        rows.append(
            (
                run["id"],
                ...
                result.get("threshold_coverage"),
                result.get("threshold_accuracy"),
                result.get("macro_f1"),
            )
        )
    ...
    for name, backbone, changed, labels, fingerprint, coverage, accuracy, macro in rows:
        ...
        if coverage is None or accuracy is None or macro is None:
            lines.append(head + "- | - | - |")
        else:
            lines.append(head + f"{coverage:.1%} | {accuracy:.1%} | {macro:.3f} |")
    lines.append("")
    to.write_text("\n".join(lines), encoding="utf-8")
    print(f"{len(rows)} runs written to {to}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--index",
        action="store_true",
        help="write runs/README.md listing every run, whatever labels it had",
    )
